...
Codex-agent-ui — 基于 Codex Agent SDK 的多终端实时同步 Agent UI。一端输入，所有连接终端实时看到输出。直接调用 Agent SDK（非 CLI 包装），会话数据存储在 SDK 的 JSONL 文件中，与 Codex CLI 双向兼容。可作为桌面应用运行（systray + auto-launch），也可嵌入外部页面（embed widget）。
...
# 单包操作
pnpm --filter @Codex-agent-ui/server dev    # tsx watch src/index.ts
pnpm --filter @Codex-agent-ui/server start  # node dist/index.js (生产模式)
pnpm --filter @Codex-agent-ui/web dev       # vite dev (5173)
pnpm --filter @Codex-agent-ui/web build     # vite build (产出 dist/ + embed.js)
pnpm --filter @Codex-agent-ui/web preview   # vite preview
pnpm --filter @Codex-agent-ui/shared build  # tsc
```
...
- `auth.ts` — AuthManager：bcrypt 密码哈希 + JWT 鉴权（12h 过期），持久化到 `~/.Codex-agent-ui/admin-auth.json`
...
## 核心原则：与 Codex CLI 行为一致

本项目的所有交互机制必须与 Codex CLI 源代码的行为**完全一致**。不做简化、不做"差不多"的近似实现。实现前必须读 CLI 源代码确认真实行为，而不是凭猜测。CLI 源代码位于 `E:\projects\Codex`。
...
直接调用 `@anthropic-ai/Codex-agent-sdk` V1 API：
...
| Layer | Tech |
|-------|------|
| Agent SDK | @anthropic-ai/Codex-agent-sdk ^0.2.97 (V1 query API) |
