...
**Files:**
- Create: `packages/server/src/agent/session.ts`
- Create: `packages/server/src/agent/v1-session.ts`

---

- [ ] **Step 1: Create agent/session.ts (EventEmitter-based interface)**

```typescript
// packages/server/src/agent/session.ts
import { EventEmitter } from 'events'
import type { SessionStatus, PermissionMode } from '@claude-agent-ui/shared'
import type { ToolApprovalRequest, ToolApprovalDecision, AskUserRequest, AskUserResponse } from '@claude-agent-ui/shared'
import type { SendOptions, SessionResult } from '@claude-agent-ui/shared'
...
- [ ] **Step 2: Create agent/v1-session.ts**

```typescript
// packages/server/src/agent/v1-session.ts
...
Expected: Server starts. V1QuerySession imports resolve correctly.

- [ ] **Step 4: Commit**

```bash
git add packages/server/src/agent/
git commit -m "feat(server): AgentSession interface + V1QuerySession (canUseTool, send, abort)"
```
